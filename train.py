...
def guidance_setup(guidance_opt):
    if guidance_opt.guidance=="SD":
        from guidance.gcs_utils import StableDiffusion
    else:
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video):
    ...
    for f in [b.decode() for b in subprocess.check_output("git ls-files", shell=True).splitlines()]:
        if not os.path.exists(f) or os.path.isdir(f):
            continue
        os.makedirs(os.path.join(code_save_folder, os.path.dirname(f)), exist_ok=True)
        shutil.copyfile(f, os.path.join(code_save_folder, f))
    ...
    target_features = gaussians.get_features
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if len(opt.grad_clip) != 0:
            ...
            gaussians.clip_grad_norm(clip_value)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    gaussians.set_target(target_features)
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold)
                    target_features = gaussians.get_t_features
            ...
            if (iteration < opt.end_reset_iter) and (iteration > opt.start_reset_iter):
                ...
                if qs > opt.lgt_thresh:
                    gaussians.reset_brightness(opt.reset_rate)
            # Optimizer step
            if iteration < opt.iterations:
...
if __name__ == "__main__":
    ...
    gaussians = training(lp, op, pp, gcp, gp, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.save_video)
